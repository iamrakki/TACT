...
function ChildModal({ warnHandleClose, data, path, isMode = false, handleModeOpen, dataFetch }) {
...
const WarningPopUp = ({ warnOpen, setWarnOpen, warnHandleOpen, warnHandleClose, data, dataFetch, path, isMode = false, handleModeOpen }) => {
    const navigate = useNavigate();
    ...
    const agreement = () => {
        if (isMode) {
            ...
            handleModeOpen(folderData);
            warnHandleClose();
        }
        else {

            navigate(`${path}/${isInFolderStatic}/${folderStatic}/${isSubStatic}/${folderIdStatic}`);
        }
    }
    return (
        <div >
            ...
            >
                ...
                <DialogActions>
                    <ChildModal warnHandleClose={warnHandleClose} data={data} path={path} isMode={isMode} handleModeOpen={handleModeOpen} dataFetch={dataFetch} />
                    <Button onClick={() => agreement()}>Agree</Button>
                </DialogActions>
            </Dialog>
        </div>
    );
